...
"jobscout:matchResultRestore",
 ...
 <h1 className=" text-5xl md:text-6xl text-primary font-black tracking-tighter leading-none">
 대시보드
 </h1>
 ...
 <span className="text-xs text-primary-foreground/60 font-bold">
 빠른 시작
 </span>
 </div>
 ...
 disabled={!url.trim()}
 className="bg-accent text-accent-foreground px-8 py-3 text-sm font-bold hover:bg-accent/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
 >
 분석 시작 →
 </button>
 </div>
 ...
 <div>
 <h2 className=" text-3xl text-primary font-black tracking-tight flex items-center gap-3">
 ...
 className="w-full p-4 bg-card border-2 border-border hover:border-secondary text-left transition-all duration-75 hover:-translate-y-0.5"
 >
 <div className={` font-black text-3xl tabular-nums ${scoreColor(entry.matchResult.score)}`}>
 ...
 <FadeIn delay={0.1}>
 <div className="space-y-4">
 <h2 className=" text-3xl text-primary font-black tracking-tight">
 분석 도구
 </h2>
 ...
 <card.icon className={`w-7 h-7 ${card.dark ?"text-accent" :"text-accent"}`} />
 <span className={`text-[10px] ${card.dark ?"text-primary-foreground/70" :"text-muted-foreground"}`}>
 {card.label}
 </span>
 </div>
 <h3 className=" text-3xl mb-2">{card.title}</h3>
 ...
 <div
 key={i}
 className=" p-6 bg-muted/50 space-y-3"
 >
