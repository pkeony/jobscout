...
export default function HistoryPage() {
 ...
 className="ml-auto px-4 text-xs text-muted-foreground hover:text-destructive transition-colors"
 >
 전체 삭제
 </button>
 )}
...
function TabButton({
 ...
 <button
 onClick={onClick}
 className={`px-5 py-3 text-xs font-bold transition-colors duration-75 ${
 active
...
function MatchList({
 ...
 <div
 className={`shrink-0 w-16 h-16 flex items-center justify-center border-2 border-border bg-card font-black text-2xl tabular-nums ${scoreColor(entry.matchResult.score)}`}
 >
 {entry.matchResult.score}
 </div>
 <div className="flex-1 min-w-0">
 <div className="flex items-center gap-2 mb-1.5 flex-wrap">
 <h3 className=" text-lg font-bold truncate">{entry.jobTitle}</h3>
 {entry.focusPosition && (
...
function AnalyzeList({
 ...
 <li key={entry.id} className="border-b border-border">
 <div className="p-5 flex items-start gap-4">
 <div className="shrink-0 w-16 h-16 flex flex-col items-center justify-center border-2 border-border bg-card">
 <span className=" font-black text-xl tabular-nums text-accent">
 {skillCount}
 </span>
