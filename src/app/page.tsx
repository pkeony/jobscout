...
export default function HomePage() {
  ...
  return (
    <main className="flex min-h-screen flex-col bg-background">
      {/* ───── Top Navigation ───── */}
      <header className="fixed top-0 w-full z-50 bg-background/80 backdrop-blur-md border-b-2 border-dashed border-primary/20 flex items-center justify-between px-6 sm:px-8 py-4">
        <Link
          href="/"
          className="font-heading text-2xl font-black italic text-secondary"
        >
          JobScout
        </Link>
        <nav className="hidden md:flex gap-8 items-center">
          <button
            ...
            className="text-secondary border-b-2 border-secondary pb-1 text-sm font-medium"
          >
            분석 도구
          </button>
          <button
            ...
            className="text-primary hover:text-secondary transition-colors duration-100 text-sm font-medium"
          >
            분석하기
          </button>
          <Link
      ...
      {/* ───── Hero Section ───── */}
      <section className="relative min-h-[820px] flex flex-col justify-center items-center px-8 pt-24 overflow-hidden dot-matrix-texture">
        {/* Background watermark */}
        <div className="absolute top-10 right-10 opacity-[0.04] select-none pointer-events-none hidden lg:block">
          <span className="text-[200px] font-heading font-black">01</span>
        </div>
        ...
        <FadeIn className="z-10 text-center max-w-4xl space-y-6">
          ...
          {/* Main title */}
          <h1 className="font-heading text-7xl md:text-8xl text-primary leading-tight font-black tracking-tighter">
            합격을
            <br />
            <span className="italic text-secondary">설계합니다</span>
          </h1>
          ...
          <div className="pt-8">
            <button
              onClick={() => router.push("/login")}
              className="relative group bg-primary text-primary-foreground px-10 py-4 text-lg uppercase tracking-widest font-medium hover:bg-primary/90 transition-colors"
            >
              입장하기
      ...
      <section id="features" className="py-24 px-6 sm:px-8 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <Link
            href="/login"
            className="md:col-span-7 bg-card p-10 border-2 border-primary/10 hover:border-secondary/50 transition-colors group"
          >
            <div className="flex justify-between items-start mb-12">
              <Search className="w-9 h-9 text-secondary" />
              <span className="text-xs uppercase text-primary/40 tracking-widest">
                MODULE_001
              </span>
            </div>
            ...
            <div className="h-1 bg-muted w-full group-hover:bg-secondary transition-colors" />
          </Link>
          ...
          >
            <div>
              <Target className="w-9 h-9 text-secondary mb-8" />
              <h3 className="font-heading text-4xl mb-4">프로필 매칭</h3>
              <p className="text-primary-foreground/60">
                내 이력서와 채용공고를 비교 분석하여 적합도 점수, 강점,
                보완점을 한눈에 파악합니다.
              </p>
            </div>
            <div className="mt-8 text-secondary text-sm tracking-widest flex items-center gap-2 group-hover:translate-x-1 transition-transform">
              매칭 분석하기 →
            </div>
          </Link>
          ...
          <Link
            href="/login"
            className="md:col-span-7 bg-card p-10 border-2 border-primary/10 relative overflow-hidden group"
          >
            <div className="absolute right-0 top-0 w-24 h-24 bg-secondary/5 -rotate-45 translate-x-12 -translate-y-12" />
            <MessageCircle className="w-9 h-9 text-secondary mb-8" />
            <h3 className="font-heading text-4xl text-primary mb-4">
              면접 예상질문
            </h3>
      ...
      <section
        id="input-section"
        className="py-24 bg-muted border-y-2 border-dashed border-primary/20"
      >
        <div className="max-w-4xl mx-auto px-6 sm:px-8">
          {/* Section header — centered */}
          <div className="mb-12 text-center">
            <h2 className="font-heading text-5xl text-primary mb-4">
              채용공고 입력하기
            </h2>
          ...
          >
            <TabsList className="bg-card p-0.5 flex mb-8 w-full">
              <TabsTrigger
                value="url"
                className="flex-1 py-4 data-active:bg-primary data-active:text-primary-foreground"
              >
                URL 입력
              </TabsTrigger>
              <TabsTrigger
            ...
            <TabsContent value="url" className="space-y-6">
              <div>
                <label className="block text-xs text-secondary mb-2 uppercase tracking-widest font-medium">
                  채용공고 URL
                </label>
                <input
              ...
              <button
                ...
                className="w-full bg-secondary text-secondary-foreground py-6 text-xl uppercase tracking-[0.2em] font-medium hover:bg-secondary/90 transition-colors flex items-center justify-center gap-4 group disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {crawlStatus === "loading"
                  ? "가져오는 중..."
                  : "분석 시작"}
      ...
      {/* ───── Bottom CTA ───── */}
      <section className="py-32 px-8 text-center bg-background dot-matrix-texture overflow-hidden">
        <div className="max-w-3xl mx-auto relative">
          {/* Watermark */}
          <div className="absolute -top-12 left-1/2 -translate-x-1/2 text-[120px] font-heading text-primary/[0.04] select-none pointer-events-none font-black hidden sm:block">
            READY?
          </div>
          <h2 className="font-heading text-5xl sm:text-6xl text-primary mb-8 relative z-10">
            취업 준비, AI와 함께
          </h2>
          ...
          <div className="flex flex-col items-center gap-6">
            <button
              onClick={() => router.push("/login")}
              className="bg-primary text-primary-foreground px-12 py-5 text-lg uppercase tracking-widest font-medium hover:scale-[1.02] transition-transform flex items-center gap-3"
            >
              지금 시작하기
            </button>
          </div>
      ...
      <footer className="w-full py-12 px-8 flex flex-col md:flex-row justify-between items-center gap-6 bg-muted border-t-2 border-primary">
        ...
        <div className="flex gap-8">
          <Link
            href="/analyze"
            className="text-xs uppercase text-primary/60 hover:text-secondary transition-colors"
          >
            채용공고 분석
          </Link>
          <Link
