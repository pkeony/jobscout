...
function ProgressStep({
 ...
 <span className=" font-black text-2xl text-accent tabular-nums">
 {num}
 </span>
...
export default function AnalyzePage() {
 ...
 className="w-full bg-accent text-accent-foreground py-5 text-lg font-medium hover:bg-accent/90 transition-colors flex items-center justify-center gap-4 group disabled:opacity-50 disabled:cursor-not-allowed"
 >
 ...
 />
 {imageFiles.length > 0 && (
 <ul className="space-y-2 border-l-2 border-input pl-4">
 {imageFiles.map((f, i) => (
 ...
 <span className="inline-block bg-accent text-accent-foreground px-5 py-2.5 text-sm font-bold">
 채용공고 분석 결과
 </span>
 <button
 onClick={handleReset}
 className="bg-muted border-2 border-input hover:border-secondary px-5 py-2.5 text-sm text-primary hover:text-accent font-bold transition-all duration-75 hover:-translate-y-0.5"
 >
 새 공고 분석 →
 </button>
 </div>
 ...
 <button
 onClick={() => router.push("/match")}
 className="group p-6 bg-muted border-2 border-border hover:border-secondary text-left transition-all duration-75 hover:-translate-y-0.5"
 >
