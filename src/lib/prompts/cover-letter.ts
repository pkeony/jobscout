...
export const PROMPT_VERSION = "cover-letter@v1.2.0-2026-04-19";
  ...
  "sections": [
    {
      "heading": "성장 경험",
      "paragraphs": [
        "[Situation] 팀 프로젝트에서 상품 검색 랭킹 개선을 맡았습니다. [Task] 기존 BM25 기반 결과가 long-tail 쿼리에서 실패하는 문제를 해결해야 했습니다. [Action] 임베딩 기반 재랭킹을 Python FastAPI 로 제공하고 MRR 지표를 A/B 로 측정했습니다. [Result] 최종적으로 long-tail 쿼리의 MRR 을 0.32 에서 0.47 로 약 47% 끌어올렸습니다."
      ]
    }
  ]
}
  ...
  "sections": [
    {
      "heading": "핵심 역량",
      "paragraphs": [
        "[Situation] 커머스 스타트업에서 월간 활성 유저 8만 규모의 React 웹 앱을 2년간 운영했습니다. [Task] 주문 플로우의 CLS 가 0.31 로 Core Web Vitals Poor 구간이었고 이를 Good 구간(<0.1)으로 끌어내는 것이 목표였습니다. [Action] Next.js App Router 로 마이그레이션하면서 이미지에 aspect-ratio 와 priority hints 를 도입하고, 결제 스텝에서 발생하던 레이아웃 시프트를 Skeleton 치환으로 제거했습니다. [Result] CLS 를 0.08 까지 낮추고 결제 페이지 전환율이 4.2%p 개선됐습니다."
      ]
    }
  ]
}

예시 4) 프로덕트 매니저 (비기술 직무 예시 — STAR 를 OKR·실험 기반으로 구성)
{
  "companyName": "라이너",
  "jobTitle": "프로덕트 매니저",
  "sections": [
    {
      "heading": "성장 경험",
      "paragraphs": [
        "[Situation] SaaS 구독 전환율이 분기 OKR 에서 12% 로 정체되어 있었습니다. [Task] 저는 온보딩 단계의 drop-off 를 줄여 전환율을 18% 이상으로 끌어올리는 과제를 맡았습니다. [Action] GA4 이벤트 funnel 을 재설계해 3번째 스텝에서 이탈이 집중된 것을 확인하고, 입력 필드를 7개에서 3개로 축소하는 A/B 실험을 4주간 운영했습니다. [Result] 전환율이 18.4% 로 상승해 OKR 목표를 초과 달성했고, 온보딩 평균 완료 시간도 2분 40초에서 1분 15초로 단축됐습니다."
      ]
    }
  ]
}`;
