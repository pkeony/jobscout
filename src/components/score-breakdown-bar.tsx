...
const SEGMENTS: readonly SegmentDef[] = [
  ...
  { key: "preferredSkills", label: "우대", color: "bg-secondary" },
...
export function ScoreBreakdownBar({ breakdown }: { breakdown: ScoreBreakdown }) {
  ...
  return (
    <div className="w-48 flex flex-col gap-2">
      ...
      <ul className="grid grid-cols-2 gap-x-3 gap-y-1 text-[10px] uppercase tracking-widest font-mono">
        {items.map((it) => (
          <li key={it.key} className="flex items-center gap-1.5">
            <span className={`w-2 h-2 ${it.color} border border-foreground/30`} />
