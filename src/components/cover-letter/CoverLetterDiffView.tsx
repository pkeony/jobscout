      ...
      {parts.map((part, i) => {
        if (part.added) {
          return (
            <span
              key={i}
              className="bg-secondary/15 text-secondary font-semibold underline decoration-secondary/40 decoration-1 underline-offset-2"
            >
              {part.value}
            </span>
          );
...
export function CoverLetterDiffView({
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between pb-3 border-b-2 border-foreground/20">
        <div>
        ...
        <div className="flex items-center gap-3 text-[10px] uppercase tracking-widest text-muted-foreground">
          <span className="flex items-center gap-1.5">
            <span className="inline-block w-3 h-3 bg-secondary/15 border border-secondary/40" />
            추가
          </span>
          <span className="flex items-center gap-1.5">
      ...
      {sectionDiffs.map((s) => (
        <section key={s.heading} className="space-y-3">
          <h4 className="font-heading text-lg font-bold italic">
              ...
              {s.notes.map((n, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p>
                      <span className="font-semibold text-secondary">v1: </span>
                      {n.after}
                    </p>
                  </div>
                );
      ...
      <div className="pt-4 border-t-2 border-foreground/20 text-[10px] uppercase tracking-widest text-muted-foreground">
